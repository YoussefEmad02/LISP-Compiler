...
def find_token(text):
    ...
    text = text.lower()  # LISP is case-insensitive
    S = text.split()
...
def ListItem(j):
    ...
    if (j < len(Tokens)) and (re.match("^\".*\"$", Tokens_Dict['Lex']) or re.match("^[0-9]\.?[0-9]*?$", Tokens_Dict['Lex'])):
        Atom_Dict = Atom(j)
...
def Term (j):
    ...
    if (j < len(Tokens)) and (re.match("^[a-zA-Z][a-zA-Z0-9]*$", Tokens_Dict['Lex']) or re.match("^[0-9]\.?[0-9]*?$", Tokens_Dict['Lex'])):
...
def BoolExpDash(j):
    ...
    elif (j < len(Tokens)) and (re.match("^[a-zA-Z][a-zA-Z0-9]*$", Tokens_Dict['Lex']) or re.match("^[0-9]\.?[0-9]*?$", Tokens_Dict['Lex'])):
        Factor_dict = Factor(j)
        Children.append(Factor_dict["node"])
        output["index"] = Factor_dict["index"]
...
def Match(a, j):
    ...
    if (j < len(Tokens)):
        ...
        if (Temp['token_type'] == a):
            ...
            output["index"] = j
            return output
        elif ((Temp['token_type'] != a) and ((Temp['Lex'] in ReservedWords) or (Temp['Lex'] in Operators) or (Temp['Lex'] in RelOperators) or (Temp['Lex'] in LogicalOperators) or (Temp['Lex'] in IncOperators) )):
